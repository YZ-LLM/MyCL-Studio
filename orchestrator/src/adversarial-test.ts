// adversarial-test — BAĞIMSIZ düşman-test yazarı (Ümit 2026-06-12: "güveni kökten sağlamlaştır"). Faz 8'de testi
...
export async function runAdversarialTester(state: State, config: MyclConfig): Promise<AdversarialResult> {
  let spec: string;
  try {
    spec = await readFile(join(state.project_root, ".mycl", "spec.md"), "utf-8");
  } catch {
