...
describe("detectMissingService — crash'ten eksik servisi tespit", () => {
  ...
  it("ECONNREFUSED :5432 → PostgreSQL", () => {
    expect(detectMissingService("connect ECONNREFUSED ::1:5432", "", SERVICES)?.name).toBe("PostgreSQL");
  });
  it("ECONNREFUSED :27017 → MongoDB", () => {
    expect(detectMissingService("MongoNetworkError: ECONNREFUSED 127.0.0.1:27017", "", SERVICES)?.name).toBe("MongoDB");
  });
  it("ECONNREFUSED :6379 → Redis", () => {
    expect(detectMissingService("Error: Redis connection ECONNREFUSED 127.0.0.1:6379", "", SERVICES)?.name).toBe("Redis");
  });

  it("port okunamayan ECONNREFUSED + package.json'da mysql2 → MySQL (imza tahmini)", () => {
    const s = detectMissingService("Error: connect ECONNREFUSED", '{"dependencies":{"mysql2":"^3.0.0","express":"^4"}}', SERVICES);
    expect(s?.name).toBe("MySQL");
  });

  it("çok ekosistemli imza: requirements.txt'te psycopg2 → PostgreSQL (artık npm'e kilitli değil)", () => {
    const s = detectMissingService("Error: connect ECONNREFUSED", "flask==3.0\npsycopg2-binary==2.9\n", SERVICES);
    expect(s?.name).toBe("PostgreSQL");
  });

  it("bilinmeyen port → jenerik servis (yine spesifik port ver)", () => {
    const s = detectMissingService("connect ECONNREFUSED 127.0.0.1:8529", "", SERVICES);
    expect(s?.port).toBe(8529);
    expect(s?.name).toContain("8529");
  });

  it("KOD hatası (ECONNREFUSED yok) → null (servis değil; Faz 0 debug'a gider)", () => {
    expect(detectMissingService("TypeError: Cannot read properties of undefined (reading 'x')", "", SERVICES)).toBeNull();
    expect(detectMissingService("SyntaxError: Unexpected token", '{"dependencies":{"mysql2":"^3"}}', SERVICES)).toBeNull();
  });
  ...
  it("YANLIŞ-POZİTİF önleme: eski/kurtarılmış ECONNREFUSED + SONRA ölümcül KOD hatası → null (servis değil)", () => {
    ...
    expect(detectMissingService(log, "", SERVICES)).toBeNull();
  });
  it("ölümcül hata GERÇEKTEN bağlantı: erken kod-uyarısı + SON ECONNREFUSED → servis tespit", () => {
    ...
    expect(detectMissingService(log, "", SERVICES)?.name).toBe("MySQL");
  });
  it("boş crash → null", () => {
    expect(detectMissingService("", "", SERVICES)).toBeNull();
  });
});
