...
// ve `error_folder/errors.db`'ye RUNTIME_ prefix'li satır olarak yazar.
//
...
export function attachRuntimeErrorWatcher(opts: AttachOpts): RuntimeErrorWatcher {
  ...
  const record = async (e: PendingEntry): Promise<void> => {
    ...
    // Debounce: aynı (error_code, location) son 5sn içinde yazıldıysa atla.
    const dedupeKey = `${errorCode}::${location}`;
    const now = Date.now();
