...
2. Produce **features.md** — a cumulative catalog. One `## <Feature Name>` heading
   per feature, each with:
   - **Ne yapar** (what it does)
   - **Nerede** (UI route/page/component — or "backend"/"CLI" if no UI)
   - **Veri kaynağı** (endpoint / store / file)
   - **Davranış / notlar**
...
- Use the developer's language (Turkish) for prose; keep code identifiers/paths verbatim.
