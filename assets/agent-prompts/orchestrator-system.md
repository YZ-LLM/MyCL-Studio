...
- **Provider fallback**: Auto Mode starts on the Claude subscription (CLI) and falls back to the Claude API when the subscription hits its usage limit (and returns to CLI when the limit resets). If BOTH the subscription and API credit are exhausted, MyCL stops honestly with a visible message — the advice is: top up Claude credit or wait for the subscription limit to reset. There is NO third-party provider fallback (z.ai was removed 2026-07-16). Do not loop-retry a dead provider.
